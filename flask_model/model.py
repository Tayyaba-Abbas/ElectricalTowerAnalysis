...
def resize_with_padding(image, target_size=(640, 640), fill_color=(0, 0, 0)):
    ...
    # Calculate the resize ratio and new size
    ...
    # Create a new image with the target size and paste the resized image onto it (centered)
...
model_path = os.path.join("model", "complete_tower_maskrcnn_resnet50_fpn_v2_augmented706_imgs_epochs320.pth")
...
def save_to_mongodb_async(email, original_img, annotated_img, insulator_height, insulator_width, pattern_count, dimensions, collection):
    def save():
        try:
            ...
            for col in ['Height (pixels)', 'Width (pixels)', 'Mapped Height (cm)', 'Mapped Width (cm)']:
                dimensions[col] = dimensions[col].apply(lambda x: x.item() if hasattr(x, 'item') else x)

            document = {
                "email": email,
                "original_image": original_base64,
                "annotated_image": annotated_base64,
                "insulator_height_px": insulator_height.item() if hasattr(insulator_height, 'item') else insulator_height,
                "insulator_width_px": insulator_width.item() if hasattr(insulator_width, 'item') else insulator_width,
                "cross_pattern_count": pattern_count,
                "dimensions": dimensions.to_dict(orient='records'),
                "timestamp": datetime.utcnow()
            }

            result = collection.insert_one(document)
            print(f"✅ MongoDB saved with _id: {result.inserted_id}")
        except Exception as e:
...
def predict():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            for col in ['Height (pixels)', 'Width (pixels)', 'Mapped Height (cm)', 'Mapped Width (cm)']:
                dimensions[col] = dimensions[col].apply(
                    lambda x: round(float(x), 2) if isinstance(x, torch.Tensor) else x)

            dimensions_list = dimensions.to_dict(orient='records')

            # 🔁 Start background MongoDB save
            save_to_mongodb_async(
                email, original_img, annotated_img,
                insulator_height, insulator_width,
                pattern_count, dimensions.copy(),
                collection  # <-- use your actual MongoDB collection object here
            )

...
export default function AllPredictionsByEmail() {
  ...
  useEffect(() => {
    const fetchUploads = async () => {
      ...
      const apiUrl = `http://localhost:3000/api/records/all?email=${encodeURIComponent(userEmail)}`;
      ...
      try {
        ...
        if (Array.isArray(data) && data.length === 0) {
          ...
          setUploads(data);
        //  localStorage.setItem("predictionCount", data.length); // ✅ Store count here
          setErrorMsg("");
        }
      } catch (err) {
    ...
    return (
      <div className="prediction-container">
        <div className="top-section">
          ...
          <p>All Analyzed Electrical Tower Images</p>
        </div>
        ...
        {uploads.map((upload, idx) => (
          <div key={upload._id || idx} className="upload-block">
            ...
            <p>
              ...
              {upload.timestamp ? new Date(upload.timestamp).toLocaleString() : "N/A"}
            </p>
              ...
              <div className="image-column">
                ...
                <div className="image-block">
                  {upload.original_image ? (
                    <img
                      src={`data:image/jpeg;base64,${upload.original_image}`}
              ...
              <div className="image-column">
                ...
                <div className="image-block">
                  {upload.annotated_image ? (
                    <img
                      src={`data:image/jpeg;base64,${upload.annotated_image}`}
                      alt={`Annotated Upload #${idx + 1}`}
                    />
                  ) : (
                  ...
                  <tbody>
                    {Object.entries(upload.dimensions).map(([key, comp]) => {
                      const heightVal = Number(
                        comp["Mapped Height (cm)"] ?? comp.height_cm
                      );
                      const widthVal = Number(
                        comp["Mapped Width (cm)"] ?? comp.width_cm
                      );
                      ...
                      return (
                        <tr key={key}>
                          ...
                          <td>
                            {!isNaN(heightVal) ? heightVal.toFixed(2) : "N/A"}
                          </td>
                          <td>
                            {!isNaN(widthVal) ? widthVal.toFixed(2) : "N/A"}
                          </td>
                        </tr>
                      );
                    })}
